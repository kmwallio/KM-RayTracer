	...
	for( $i = $startY ; $i > $endY ; $i-- ){ # Outer loop is Y to make writing to PPM Format Easier.
		for( $j = $startX ; $j < $endX ; $j++){
			...
			foreach $curObj (@{$self->{_kmobjs}}){
				...
				if( $closestT < 0 || $closestT > $newT ){ # See if it's the first item or a closer one.
			...
			if( $closestT >= 0 ){
				...
				push(@{$self->{_image}}, $closestObj->getColor($closestT, $self->{_kmobjs}));
			}else{
